...
idle(Browser) ->
	...
	{Browser, #{<<"join">> := Who}} ->
	    ...
	    idle(Browser);
	{irc, welcome, Who} ->
	    Browser ! #{cmd => hide_div, id => idle, txt => ""},
	    Browser ! #{cmd => show_div, id => running, txt => ""},
	    running(Browser, Who);
	X ->
...
running(Browser, Who) ->
    receive
	{new_browser, NewBrowser} ->
		running(NewBrowser, Who);
	{Browser,#{<<"entry">> := <<"tell">>, <<"txt">> := Txt}} ->
	    irc ! {broadcast, Who, Txt},
	    running(Browser, Who);
	{Browser,#{<<"clicked">> := <<"Leave">>}} ->
	    irc ! {leave, Who},
	    Browser ! #{cmd => hide_div, id => running, txt => ""},
	    Browser ! #{cmd => show_div, id => idle, txt => ""},
	    idle(Browser);
	{irc, scroll, Bin} ->
